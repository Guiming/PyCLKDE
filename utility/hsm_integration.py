...
def main(flag):
    ...
    RES = '90m'
    # project directory
    ProjDir = 'D:\OneDrive - University of Denver\Dropbox\PyCLKDEHSM'
    import platform
    if platform.node() == 'DESKTOP-GQE57MS':
        ProjDir = 'D:\Dropbox\PyCLKDEHSM'

    # elevation and hillshade
    hillfn = ProjDir + os.sep + r'data\base\hillshade.tif'
    if RES == '30m':
        hillfn = hillfn.replace('.tif', '30m.tif')
    elevationfn = ProjDir + os.sep + r'data\base\elevation.tif'
    if RES == '30m':
        elevationfn = elevationfn.replace('.tif', '30m.tif')

    '''
    ### hillshade for vaisualization
    hillmap = raster.Raster()
    hillmap.readRasterGDAL(hillfn, pdf=False)
    print("here")
    ### elevation
    elevationmap = raster.Raster()
    elevationmap.readRasterGDAL(elevationfn, pdf=False)
    '''

    # ascii data directories
    tifDir = ProjDir + os.sep + 'data' + os.sep + 'covariates'
    if RES == '30m':
        tifDir = tifDir + os.sep + '30m'
    tiffns = ['elevation.tif', 'slopetan.tif', 'aspectcos.tif', 'aspectsin.tif', 'snowdepth2009.tif', 'landcover.tif']
    #'canopycoverage.tif', 'edgelengthr270.tif', 'snowdepth2009.tif',

    samplefnA = ProjDir + os.sep + 'data' + os.sep + 'localities' + os.sep + 'deer_2009_sample_format.csv'
    if RES == '30m':
        samplefnA = samplefnA.replace('.csv', '_30m.csv')
    samplefnB = ProjDir + os.sep + 'data' + os.sep + 'localities' + os.sep + 'deer_2010_sample_format.csv'
    if RES == '30m':
        samplefnB = samplefnB.replace('.csv', '_30m.csv')

    ### read in species occurrences
    samplesA = points.Points()
    samplesA.readFromCSV(samplefnA)
    #samplesA.plot()
    #samplesA.plot_background(elevationmap, hillmap, title = 'deer 2009', labelpnts = False)

    samplesB = points.Points()
    samplesB.readFromCSV(samplefnB)
    #samplesB.plot()
    #samplesB.plot_background(elevationmap, hillmap, title = 'deer 2010', labelpnts = False)

    samplesAll = samplesA.mergePoints(samplesB).eliminateDuplicates()
    #samplesAll.plot()
    #samplesAll.plot_background(elevationmap, hillmap, title = 'deer all', labelpnts = False)
    ...
    points_bkg = points.Points() # for computing background (population) pdf
    points_bkg.generateRandom(1000, envrasters[0])

    if False and RES == '30m':
        rnd5000 = points.Points()
        rnd5000.generateRandom_v2(5000, envrasters, val = 0)
        rnd5000.writeCSV(ProjDir + os.sep + 'data' + os.sep + 'localities' + os.sep + 'background_pnts_5000_30m.csv')
    '''
    for rst in envrasters:
        rst.plot()
        samplesAll.plot_density(rst, hillmap)
    samplesAll.plot_density_curves(envrasters, points_bkg=points_bkg)
    '''

    #sys.exit(0)

    evweights = np.ones(len(envrasters))
    predictors = envrasters
    ratios = evweights
    print( 'done reading environmental rasters...')

    ## generate random background samples needed in plot roc
    bkg_samples_fn = ProjDir + os.sep + 'data' + os.sep + 'localities' + os.sep + 'background_pnts_5000.csv'
    if RES == '30m':
        bkg_samples_fn = bkg_samples_fn.replace('.csv', '_30m.csv')
    bkg_samples = points.Points()
    bkg_samples.readFromCSV(bkg_samples_fn)
    #bkg_samples.plot_background(elevationmap, hillmap, title = 'random background samples', labelpnts = False)
    print ('done reading background samples...')

    ## suitability mapping with unweighted pcs
    #hsm = HSM(predictors, [samplesAll], samplepdfonly=False, points_bkg=points_bkg)
    #hsm.plot_roc_curve(test_samples = samplesAll, bkg_samples = bkg_samples, title = 'ROC Curve')

    '''
    hsmap = hsm.suitabilityMap()[0]
    hsmap.filename = 'suitabilityA'
    hsmap.plot(title = 'suitabilityA', hillshade = hillmap)
    #hsmap.writeRasterGDAL('suitabilityA.tif')
    '''

    #hsm.suitabilityMapTile(suitabilityfn='hsvnp.tif')
    #util.printTimes()
    ...
    if flag == 0:
        ...
        hsm = HSM(predictors, [samplesA], samplepdfonly=False, points_bkg=points_bkg)
        #hsm.plot_roc_curve(test_samples = samplesA, bkg_samples = bkg_samples, title = SPECIES.split('_')[0] + 'ROC Curve_ebd31')
        ### for cases where covariates are read in tiles
        hsm.suitabilityMapTile(suitabilityfn='hs_' + SPECIES.split('_')[0] + '_all31.tif')


    elif flag == 1:
        hsm = HSM(predictors, [samplesA, samplesB], samplepdfonly=False, integration_level = 'data', points_bkg=points_bkg)
        #hsm.plot_roc_curve(test_samples = samplesAll, bkg_samples = bkg_samples, title = SPECIES.split('_')[0] + 'ROC Curve__ebdinat_data31')
        hsm.suitabilityMapTile(suitabilityfn='hs_' + SPECIES.split('_')[0] + '_data31.tif')

    elif flag == 2:
        hsm = HSM(predictors, [samplesA, samplesB], samplepdfonly=False, integration_level = 'model', integration_strategy = 'minimum', points_bkg=points_bkg)
        #hsm.plot_roc_curve(test_samples = samplesAll, bkg_samples = bkg_samples, title = SPECIES.split('_')[0] + 'ROC Curve_ebdinat_modelmin31')
        hsm.suitabilityMapTile(suitabilityfn='hs_' + SPECIES.split('_')[0] + '_modelmin31.tif')
    else: # flag == 3
        hsm = HSM(predictors, [samplesA, samplesB], samplepdfonly=False, integration_level = 'knowledge', integration_strategy = 'minimum', points_bkg=points_bkg)
        #hsm.plot_roc_curve(test_samples = samplesAll, bkg_samples = bkg_samples, title = SPECIES.split('_')[0] + 'ROC Curve_ebdinat_knowedgemin31')
        hsm.suitabilityMapTile(suitabilityfn='hs_' + SPECIES.split('_')[0] + '_knowledgemin31.tif')

if __name__ == "__main__":
    txxx = time.time()
    timesALL = {}
    for bw in [0, 1, 2]:
        conf.BW_OPTION_OCCR = bw

        for k in [True, False]:
            conf.OPENCL_KDE = k
            if k:
                for plt, PLT in zip(['cpu', 'gpu'], [{'Platform': 'Intel(R) OpenCL', 'Device':'Intel(R) Core(TM) i9-8950HK CPU @ 2.90GHz'}, {'Platform': 'NVIDIA CUDA', 'Device':'Quadro P4000'}]):
                    conf.OPENCL_CONFIG = PLT
                    for flag in list(range(0, 4)):
                        FLAG = 'BW' + str(bw) + '_CL' + str(k) + '_DEV' + plt + '_INTG' + str(flag)
                        times = {}
                        conf.TIME_ACC_WRITES = []
                        conf.TIME_ACC_READS = []
                        conf.TIME_ACC_KDE_BKG = []
                        conf.TIME_ACC_KDE_SMPL = []

                        conf.time_acc_reads = 0.0
                        conf.time_acc_writes = 0.0
                        conf.time_acc_kde_bkg = 0.0
                        conf.time_acc_kde_smpl = 0.0

                        T0 = time.time()
                        main(flag)
                        times["0total"] = time.time() - T0
                        print('++++ It took %f s in total' % (time.time() - T0))

                        times["1reads"] = conf.time_acc_reads/1000.0
                        times["2writes"] = conf.time_acc_writes/1000.0
                        times["3kde_bkg"] = conf.time_acc_kde_bkg/1000.0
                        times["4kde_smpl"] = conf.time_acc_kde_smpl/1000.0

                        timesALL[FLAG] = times

                        #with open('logs' + os.sep + platform.node() + '_times_' + FLAG + '.json', 'w') as outfile:
                        #    json.dump(times, outfile)
            else:
                for flag in list(range(0, 4)):
                    FLAG = FLAG = 'BW' + str(bw) + '_CL' + str(k) + '_DEVpy' + '_INTG' + str(flag)
                    times = {}
                    conf.TIME_ACC_WRITES = []
                    conf.TIME_ACC_READS = []
                    conf.TIME_ACC_KDE_BKG = []
                    ...
                    #    json.dump(times, outfile)
                    #util.printTimes()
    with open('logs' + os.sep + platform.node() + '_timesAll.json', 'w') as outfile:
        json.dump(timesALL, outfile)
    print('++++ It took %f s in total' % (time.time() - txxx))
